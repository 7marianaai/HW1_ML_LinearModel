...
def format_price(x: float) -> str:
    try:
        return f"{float(x):,.0f} руб.".replace(",", " ")
    except Exception:
        return str(x)
...
with st.sidebar:
    ...
    if uploaded_file is not None:
        ...
        st.info("CSV нужен только для исследовательского анализа данных - EDA. Остальные вкладки работают без него.")
...
with tabs[0]:
    ...
    if df is None:
        st.info("Загрузите CSV в sidebar — тогда появятся графики EDA.")
    else:
        st.write("Данные:")
...
with tabs[2]:
    st.subheader("Важность признаков в модели (Top-30)")
